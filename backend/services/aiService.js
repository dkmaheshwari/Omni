...
class AIService {
  ...
  checkRateLimit(userId, endpoint) {
    ...
    if (recentRequests.length >= limit) {
      console.warn(
        `Rate limit exceeded for user ${userId} on endpoint ${endpoint}: ${recentRequests.length}/${limit}`,
      );
      return false;
    }
  ...
  async askTutorQuestion(
    ...
    try {
      return await this.callWithRetry(async () => {
        return await this.aiClient.generateTutoringResponse(
          question,
          context,
          learningStyle,
          options,
  ...
  async generateLearningPath(userId, subject, currentLevel, goals) {
    ...
    try {
      ...
      console.error(
        `Learning path generation failed for user ${userId}:`,
        error,
      );
      throw new Error(this.formatError(error));
    }
  ...
  async generateEnhancedStudyGuide(
    ...
    try {
      ...
      console.error(
        `Enhanced study guide generation failed for user ${userId}:`,
        error,
      );
      throw new Error(this.formatError(error));
    }
  ...
  async generatePracticeProblems(
    ...
    try {
      ...
      console.error(
        `Practice problems generation failed for user ${userId}:`,
        error,
      );
      throw new Error(this.formatError(error));
    }
  ...
  async detectLearningStyle(userId, interactionHistory) {
    ...
    try {
      ...
      console.error(
        `Learning style detection failed for user ${userId}:`,
        error,
      );
      throw new Error(this.formatError(error));
    }
  ...
  async analyzeKnowledgeGaps(userId, subject, assessmentResults) {
    ...
    try {
      ...
      console.error(
        `Knowledge gaps analysis failed for user ${userId}:`,
        error,
      );
      throw new Error(this.formatError(error));
    }
